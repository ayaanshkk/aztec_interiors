...
            # Update attributes (excluding stage update logic for drag-and-drop PUT)
            project.project_name = data.get('project_name', project.project_name)
            project.project_type = data.get('project_type', project.project_type)
            # REMOVED: project.stage = data.get('stage', project.stage) <--- Should only be done via PATCH /stage
...
            
            # Optionally sync customer stage if this is the only linked entity (Unchanged logic)
            customer = project.customer
            current_stage = project.stage # Use current stage for sync check
            if customer:
...
                
                if total_linked == 0 and customer.stage != current_stage:
                    customer.stage = current_stage
                    customer.updated_at = datetime.utcnow()
                    note_entry_cust = f"\n[{datetime.utcnow().isoformat()}] Stage synced from {old_stage} to {current_stage} by {project.updated_by}. Reason: Linked project updated."
                    customer.notes = (customer.notes or '') + note_entry_cust
                    session.add(customer)
